...
import ClientFeeIndexComponent from './Fees';
...
class ClientFeeIndex extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    ...
    this.serverRequest = $.ajax({
      type: "GET",
      url: `${API_URL}/api/clients/update/${this.props.params.id}/fees`,
      beforeSend: (xhr) => {xhr.setRequestHeader("Authorization", Auth.getToken());}
    })
      .done((fees) => {
        // console.log('fees', fees);
        TABLE_DATA = fees.map((fee) => {
          // const deleteTo = `/clients/delete/${this.props.params.id}/contacts/delete/${contact.contact_id}`;
          const updateTo = `/clients/update/${this.props.params.id}/fees/update/${fee.cfee_id}`;
          return Object.assign({}, fee, { action: (
  ...
  handleFilterValueChange(value) {
    const itemsFiltered = _.filter(TABLE_DATA, (fee) => {
      const regExp = new RegExp(`${value}`, 'i'); // case insensitive
      const match = regExp.test(fee.notes);
      return match;
    });
  ...
  handleDeleteClick(id) { // eslint-disable-line no-unused-vars
    this.setState( {
      deletingItem: _.find(TABLE_DATA, (item) => (item.cfee_id == id)),
      dialogOpen: true
    });
  }

  handleClose() { // eslint-disable-line no-unused-vars
    this.setState({deletingItem: {}, dialogOpen: false});
  }

  handleConfirmDelete() { // eslint-disable-line no-unused-vars
    const clientFeeId = this.state.deletingItem.cfee_id;
    this.serverRequestOnDelete = $.ajax({
      type: "POST",
      url: `${API_URL}/api/fees/delete`,
      contentType: "application/json",
      dataType: "json",
      data: JSON.stringify({cfee_id: clientFeeId}),
      beforeSend: (xhr) => {xhr.setRequestHeader("Authorization", Auth.getToken());},
      success: () => {
        _.remove(TABLE_DATA, (item) => {return(item.cfee_id == clientFeeId);});
        const paginatedItems = Helper.getPaginatedItems(
          TABLE_DATA,
          this.state.perPage,
          this.state.page
  ...
  render() {
    ...
    const deleteDialog = (
      ...
      >
        Are you sure want to delete fee: {deletingItem.cfee_id} ?
      </Dialog>);

    return(
      <div>
        <ClientFeeIndexComponent
          client={client}
...
ClientFeeIndex.contextTypes = {
  router: PropTypes.object.isRequired
};

ClientFeeIndex.propTypes = {
  params: PropTypes.object.isRequired
};

export default ClientFeeIndex;
